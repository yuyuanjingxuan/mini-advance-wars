...
| 🪖 **8 unit types** | 🪖 Infantry (versatile, can capture) · 🛡️ Heavy (high ATK/DEF, anti-vehicle) · 🚙 Recon (fast raider) · 💣 Cannon (range 2-3) · 🔧 Engineer (1.5× capture + repair) · ⚕️ Medic (heals) · Tank (assault main force) · 🚀 Rocket (range 3-5) |
| 🥾 **Per-unit movement** | Advance Wars style: infantry forest 1/mountain 2, treads (heavy/tank/cannon) forest 2 & no mountains, tires (recon/rocket) plain 2/forest 3 & no mountains |
...
| 🌲 **Terrain effects** | 🌲 Forest +20% DEF · ⛰️ Mountain +40% DEF (vehicles blocked) · 🌊 River impassable (fords passable) · 🏙️ City/🏭 Factory +30% DEF · 🚩 HQ +40% DEF, owned buildings heal 2 HP per turn |
| 🚩 **Building capture** | Advance Wars-style capture: cities/factories/HQ, only infantry/engineers can capture (engineer 1.5×), stand on a building and wait to build progress (progress = current HP × multiplier, 20 to capture, progress resets when leaving); captured buildings fly your flag and can be taken back; capturing all buildings also wins — **capturing the enemy HQ wins instantly** |
...
1. **Goal**: Destroy all enemy units, or capture all buildings (cities/factories/HQ) — capturing the enemy HQ wins instantly
